...
def run_worker(queue_in, queue_out, paramiko_exception):
    while True:
        job = queue_in.get()
        my_tuple = process(job, paramiko_exception)
...
for i in range(nb_of_threads):
    t = Process(target = run_worker, args = (dictionary_of_queues["queue_of_devices_" + str(i)], print_queue, paramiko_exception))
    t.daemon = True
    t.start()
